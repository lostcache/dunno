...
async fn test_hierarchy_context() -> anyhow::Result<()> {
    ...
    // 2. Create Hierarchy
    let project = db.create_project(&Project {
        id: None,
        name: project_name,
    ...
    let module = db.create_module(&Module {
        id: None,
    ...
    let has_module_style = context.iter().any(|v| v["description"] == "Module Level Style");
    let has_project_mistake = context.iter().any(|v| v["content"] == "Project Level Mistake");
