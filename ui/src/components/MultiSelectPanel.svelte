...
{#if nodes.length >= 2}
  <div
    class="absolute bottom-0 inset-x-0 px-4 py-[10px] bg-[#14172a] border-t border-[#2d3148] max-h-[200px] overflow-y-auto text-xs z-10"
  >
    <div class="flex items-center justify-between mb-1">
      <h3 class="text-[#a78bfa]">{nodes.length} nodes selected</h3>
      <Button
        size="sm"
        class="bg-green-700 hover:bg-green-600 text-white h-6 px-2.5 text-xs"
        onclick={linkSelected}>Link</Button
      >
    </div>
    <div class="mt-2">
      {#each pairs as row}
        <div
          class="py-1 border-b border-[#2d3148] last:border-b-0 text-[11px] {row.noPair
            ? 'text-[#475569]'
            : 'text-[#94a3b8]'}"
        >
