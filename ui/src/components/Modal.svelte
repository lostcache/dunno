<script lang="ts">
  ...
  import { SCHEMAS, EDIT_SCHEMAS, EDIT_ENDPOINTS, CREATE_ENDPOINTS } from "../lib/constants";
...
>
  <Dialog.Content
    class="bg-[#1a1d27] border-[#3d4165] min-w-[360px] max-w-[500px] w-[90%] sm:max-w-[500px]"
  >
    {#if $modalState.mode === "create"}
      ...
      <div class="flex flex-col gap-2.5 max-h-[60vh] overflow-y-auto py-1 pr-1">
        {#each SCHEMAS[$modalState.tab!] ?? [] as f}
          <div>
            <Label class="text-[#94a3b8] text-xs mb-1 block"
    ...
    {:else if $modalState.mode === "edit" && $modalState.editingNode}
      <Dialog.Header>
        <Dialog.Title class="text-[#a78bfa]"
          >Edit {$modalState.editingNode.node_type}: {$modalState.editingNode.label}</Dialog.Title
        >
      </Dialog.Header>
      <div class="flex flex-col gap-2.5 max-h-[60vh] overflow-y-auto py-1 pr-1">
