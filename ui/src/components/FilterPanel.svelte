<script lang="ts">
  ...
  import { hiddenNodeTypes, hiddenEdgeTypes, toggleNodeType, toggleEdgeType } from '../stores/filterStore'
  ...
  <div class="flex flex-wrap gap-6 px-4 py-2 border-b border-[#2d3148] bg-[#1a1d27] flex-shrink-0">
    <div class="flex flex-wrap gap-2 items-center">
      ...
      {#each Object.entries(NODE_COLORS) as [type, color]}
        <Label class="flex items-center gap-1 text-[11px] cursor-pointer text-[#cbd5e1] select-none font-normal">
          <Checkbox
            checked={!$hiddenNodeTypes.has(type)}
            onCheckedChange={(v) => toggleNodeType(type, v as boolean)}
            class="size-3 rounded-none"
          />
          <span class="w-2.5 h-2.5 rounded-sm inline-block flex-shrink-0" style="background:{color.bg}"></span>
          {type}
        </Label>
      {/each}
    </div>
