<script lang="ts">
  ...
  async function loadItems() {
    ...
    else if (tab === "issues" && pid) url = `/api/issues?project_id=${pid}`;
...
<aside
  class="w-[220px] shrink-0 bg-[#14172a] border-r border-[#2d3148] flex flex-col overflow-hidden"
>
  <div class="flex flex-col overflow-y-auto flex-1 py-2">
    {#each ENTITY_TABS as tab}
      <button
        ...
        onclick={() => activeTab.set(tab)}>{tab}</button
      >
    {/each}
  </div>
  ...
  <div class="px-3 py-2.5 border-t border-[#2d3148]">
    <Button class="w-full text-xs h-8" onclick={() => openCreate($activeTab)}>+ Create</Button>
  </div>
</aside>
