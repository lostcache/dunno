<script lang="ts">
  ...
  function getExtraFields(node: Record<string, unknown>): [string, unknown][] {
    ...
    return Object.entries(node).filter(
      ([k, v]) =>
        k !== "id" && k !== "label" && k !== "node_type" && v !== node.label && !schemaKeys.has(k),
    );
  }
  ...
  async function saveNode() {
    ...
    if (!endpoint) {
      setStatus("No endpoint for " + node.node_type, "err");
      return;
    }

    const body = node.node_type === "context" ? { fields: { ...fieldValues } } : { ...fieldValues };
  ...
  async function deleteNode() {
    ...
    if (!base) {
      setStatus("No delete endpoint for " + data.node_type, "err");
      return;
    }
    try {
...
{#if $editingNode}
  <div
    class="max-lg:hidden relative flex flex-col border-l border-[#2d3148] bg-[#14172a] overflow-hidden shrink-0 h-full"
  ...
  >
    ...
    <div class="flex-1 min-h-0 overflow-y-auto px-3 pt-3 pb-2 flex flex-col gap-3">
        ...
        <div class="flex flex-col gap-1 shrink-0">
          ...
          {#if confirmingDelete}
            <div class="flex flex-col gap-1">
              <Button size="sm" variant="destructive" class="h-6 px-2 text-xs" onclick={deleteNode}
                >Confirm</Button
              >
              <Button
                size="sm"
        ...
        {#each EDIT_SCHEMAS[$editingNode.node_type] ?? [] as f}
          <div class="flex flex-col gap-1">
            ...
            {:else if f.type === "select"}
              ...
              >
                <Select.Trigger
                  class="w-full h-8 bg-[#252840] border-[#3d4165] text-[#e2e8f0] text-[13px]"
                >
        ...
        {#if getExtraFields($editingNode).length > 0}
          ...
          {#each getExtraFields($editingNode) as [key, value]}
            <div class="flex flex-col gap-0.5">
              <span class="text-[10px] text-[#64748b] uppercase tracking-wide"
      ...
      <div class="mt-auto pt-2 border-t border-[#2d3148]">
        <Button
          onclick={() =>
            navigator.clipboard.writeText($editingNode.id).then(() => setStatus("ID copied", "ok"))}
