...
<nav class="flex items-center gap-3 px-4 py-2 bg-[#1a1d27] border-b border-[#2d3148] flex-shrink-0">
  ...
  <Select.Root
    type="single"
    value={$projectId ?? ""}
    onValueChange={(v) => projectId.set(v || null)}
  >
    <Select.Trigger
      class="h-8 bg-[#252840] border-[#3d4165] text-[#e2e8f0] hover:bg-[#3d4165] text-xs w-48"
    >
      {selectedProjectName}
    </Select.Trigger>
    <Select.Content>
  ...
  <Button
    variant="ghost"
    size="sm"
    class="h-8 px-3 border border-[#3d4165] text-[#e2e8f0] hover:bg-[#3d4165] text-xs {$filterPanelOpen
      ? 'bg-[#5b45d6] border-[#7c6df0]'
      : 'bg-[#252840]'}"
    onclick={() => filterPanelOpen.update((v) => !v)}>Filter</Button
  >
  ...
  <div class="ml-auto flex gap-1.5">
    <Button
      ...
      size="sm"
      class="h-8 px-3 text-xs {$mainView !== 'graph'
        ? 'border-[#3d4165] text-[#e2e8f0] bg-[#252840] hover:bg-[#3d4165]'
        : ''}"
      onclick={() => mainView.set("graph")}>Graph</Button
    >
    <Button
      variant={$mainView === "ctx" ? "default" : "outline"}
      size="sm"
      class="h-8 px-3 text-xs {$mainView !== 'ctx'
        ? 'border-[#3d4165] text-[#e2e8f0] bg-[#252840] hover:bg-[#3d4165]'
        : ''}"
      onclick={() => mainView.set("ctx")}>Context</Button
    >
  </div>
</nav>
