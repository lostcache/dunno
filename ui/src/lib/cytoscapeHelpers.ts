...
export function truncateLabel(str: string, maxChars = 80): string {
  ...
  const normalized = str
    ...
    .replace(/\s+/g, ' ')
    .trim()
  const trimmed = normalized.length > maxChars ? normalized.slice(0, maxChars - 1) + '…' : normalized
...
export function buildNodeCardHtml(data: NodeData, selected: boolean, cardHeight: number): string {
  ...
  return `<div style="width:${NODE_CARD_W}px;height:${cardHeight}px;border-radius:6px;overflow:hidden;box-shadow:0 2px 10px rgba(0,0,0,.55);font-family:ui-sans-serif,system-ui,sans-serif;${outline}">` +
