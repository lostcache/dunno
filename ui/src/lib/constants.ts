...
export const SCHEMAS: Record<string, SchemaField[]> = {
  ...
  modules: [
    ...
    },
    {
      ...
      label: "Parent Module ID (optional)",
      type: "text",
    },
  ],
  files: [
...
export const EDIT_SCHEMAS: Record<string, SchemaField[]> = {
  ...
  todo_item: [{ name: "content", label: "Content", type: "textarea" }],
  user_story: [
...
export function findEdgePair(typeA: string, typeB: string): EdgePair | undefined {
  return EDGE_PAIRS.find(
    (p) => (p.a === typeA && p.b === typeB) || (p.a === typeB && p.b === typeA),
  );
}
